...
http.createServer((req,res)=>{let fp=path.join(PUBLIC,req.url.split('?')[0]);fs.stat(fp,(err,stat)=>{if(err){res.writeHead(404);res.end();return}if(stat.isDirectory()){fp=path.join(fp,'index.html');return fs.stat(fp,(e2,s2)=>{if(e2){res.writeHead(404);res.end();return}serve(req,res,fp,s2)})}serve(req,res,fp,stat)})}).listen(PORT,()=>console.log('http://127.0.0.1:'+PORT+' (Range: yes)'));
